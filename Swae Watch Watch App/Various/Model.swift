...
class Model: NSObject, ObservableObject {
    ...
    private func updatePreview() {
        ...
        if latestThermalStateTime < deadline,
    ...
    private func handleChatMessage(_ data: Any) throws {
        ...
        chat.posts.prepend(
            ChatPost(
                ...
                timestamp: message.timestamp,
                highlight: message.highlight.map {
                    ChatPostHighlight.fromWatchProtocol(highlight: $0)
                })
        )
    ...
    func findScoreboardPlayer(id: UUID) -> String {
        return padel.players.first(where: { $0.id == id })?.name ?? "🇸🇪 Swae"
    }
    ...
    func skipCurrentChatTextToSpeechMessage() {
        let message = WatchMessageFromWatch.pack(
            type: .skipCurrentChatTextToSpeechMessage, data: true)
        WCSession.default.sendMessage(message, replyHandler: nil)
    }
        ...
        for type in collectedTypes {
            ...
            DispatchQueue.main.async {
                ...
                case HKQuantityType.quantityType(forIdentifier: .activeEnergyBurned):
                    if let activeEnergyBurned = statistics.sumQuantity()?.doubleValue(
                        for: .kilocalorie())
                    {
