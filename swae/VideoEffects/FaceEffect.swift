...
final class FaceEffect: VideoEffect {
    ...
    let swaeImage: CIImage?
    let swaeImageMetalPetal: MTIImage?
    ...
    init(fps: Float) {
        ...
        if let image = UIImage(named: "AppIconNoBackground"), let image = image.cgImage {
            swaeImage = CIImage(cgImage: image)
            swaeImageMetalPetal = MTIImage(cgImage: image, isOpaque: true)
        } else {
            swaeImage = nil
            swaeImageMetalPetal = nil
        }
        super.init()
    }
    ...
    private func createFacesMaskImage(imageExtent: CGRect, detections: [VNFaceObservation])
        -> CIImage?
    {
        var facesMask = CIImage.empty().cropped(to: imageExtent)
        for detection in detections {
            let faceBoundingBox = CGRect(
                x: detection.boundingBox.minX * imageExtent.width,
            ...
            let faceCenter = CGPoint(
                x: faceBoundingBox.maxX - (faceBoundingBox.width / 2),
            ...
            guard
                let faceMask = faceMask.outputImage?.cropped(
                    to: faceBoundingBox.insetBy(
    ...
    private func addMouth(image: CIImage?, detections: [VNFaceObservation]?) -> CIImage? {
        guard let image, let detections, let swaeImage else {
            return image
        }
            ...
            let swaeImage =
                ...
                .transformed(
                    by: CGAffineTransform(
                        scaleX: diffX / swaeImage.extent.width,
                        y: diffX / swaeImage.extent.width
                    ))
            let offsetY = minY + (diffY - swaeImage.extent.height) / 2
            outputImage =
                swaeImage
    ...
    override func execute(_ image: CIImage, _ info: VideoEffectInfo) -> CIImage {
        ...
        if settings.showCrop {
            ...
            outputImage = outputImage?
                .cropped(
                    to: CGRect(
                        x: smallOffsetX, y: smallOffsetY, width: smallWidth, height: smallHeight)
                )
    ...
    private func createFacesMaskImageMetalPetal(
        imageExtent: CGRect,
        detections: [VNFaceObservation]
    ) -> MTIImage? {
        ...
        for detection in detections {
            ...
            let faceMask = MTIImage.radialGradient(
                size: .init(
    ...
    private func applyFacesMaskMetalPetal(
        backgroundImage: MTIImage?, image: MTIImage?,
        detections: [VNFaceObservation]?
    ) -> MTIImage? {
        guard let image, let detections else {
            return image
        ...
        guard
            let faceMasksImage = createFacesMaskImageMetalPetal(
    ...
    {
        guard let image, let detections, let swaeImageMetalPetal else {
            return image
        }
        var outputImage = image
        for detection in detections {
            ...
            let scale = diffX / swaeImageMetalPetal.extent.width
            let swaeImageMetalPetal = swaeImageMetalPetal.resized(
                to: .init(
                    width: scale * swaeImageMetalPetal.size.width,
                    height: scale * swaeImageMetalPetal.size.height
                ))
            guard let swaeImageMetalPetal else {
                continue
            }
            let offsetX = minX + swaeImageMetalPetal.size.width / 2
            let offsetY = image.size.height - minY - diffY + swaeImageMetalPetal.size.height / 2
            let filter = MTIMultilayerCompositingFilter()
            filter.inputBackgroundImage = outputImage
            filter.layers = [
                .init(content: swaeImageMetalPetal, position: .init(x: offsetX, y: offsetY))
            ]
    ...
    private func addBeautyMetalPetal(_ image: MTIImage?, _ detections: [VNFaceObservation]?)
        -> MTIImage?
    {
        var image = image
    ...
    private func addBeautyShapeMetalPetal(_ image: MTIImage?, _ detections: [VNFaceObservation]?)
        -> MTIImage?
    {
        guard let image, var detections else {
            return image
