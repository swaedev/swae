...
private let swaeScript = """
    class Swae {
      ...
      send(message) {
        window.webkit.messageHandlers.swae.postMessage(JSON.stringify(message));
      }
    }
...
class BrowserEffectServer: NSObject {
    ...
    func addScript(configuration: WKWebViewConfiguration) {
        ...
        configuration.userContentController.add(self, name: "swae")
    }
    ...
    private func send(message: MessageToBrowser) {
        do {
            ...
            webView?.evaluateJavaScript(
                """
                swae.handleMessage(window.atob("\(data)"))
                """)
        } catch {
