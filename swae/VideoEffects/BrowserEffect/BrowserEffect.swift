...
private let browserQueue = DispatchQueue(label: "com.suhail.widget.browser")
...
final class BrowserEffect: VideoEffect {
    ...
    func setSceneWidget(sceneWidget: SettingsSceneWidget?, crops: [WidgetCrop]) {
        ...
        if enabled {
            ...
            self.crops = crops.map {
                WidgetCrop(
                    position: .init(
                        x: toPixels($0.position.x, videoSize.width),
            ...
            cropsMetalPetal = crops.map {
                WidgetCrop(
                    position: .init(
                        x: toPixels($0.position.x, videoSize.width) + $0.crop.width / 2,
