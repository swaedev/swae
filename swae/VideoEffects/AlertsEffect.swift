...
private let lockQueue = DispatchQueue(label: "com.suhailSwae.Alerts")
        ...
        DispatchQueue.global().async {
            ...
            case let .bundledName(name):
                if let url = Bundle.main.url(
                    forResource: "Alerts.bundle/\(name)", withExtension: "gif")
                {
                ...
                let animatedImage = SDAnimatedImage(data: data)
            {
                for index in 0..<animatedImage.animatedImageFrameCount {
                    if let cgImage = animatedImage.animatedImageFrame(at: index)?.cgImage {
                        ...
                        images.append(
                            GifImage(image: CIImage(cgImage: cgImage), timeOffset: timeOffset))
                    }
                }
            }
        }
...
final class AlertsEffect: VideoEffect, @unchecked Sendable {
    ...
    @MainActor
    private func playTwitchSubscriptionGift(
    ...
    @MainActor
    private func playTwitchResubscribe(
    ...
    private func playTwitchCheer(event: TwitchEventSubChannelCheerEvent) {
        for (index, cheerBit) in settings.twitch!.cheerBits!.enumerated()
        where cheerBit.alert.enabled {
    ...
    private func playChatBotCommand(command: String, name: String, prompt: String) {
        guard
            let commandIndex = settings.chatBot!.commands
        ...
        case .imagePlayground:
            createImagePlaygroundImage(
                soundUrl: medias.soundUrl, settings: settings, name: name, prompt: prompt)
        }
    }
            ...
            Task {
                do {
                    ...
                    for try await image in creator.images(
                        for: concepts, style: .animation, limit: 1)
                    {
                        let medias = Medias()
                        let factor = 0.35
                        var image = CIImage(cgImage: image.cgImage).transformed(
    ...
    private func playSpeechToTextString(id: UUID) {
        guard
            let stringIndex = settings.speechToText!.strings.firstIndex(where: {
                $0.id == id && $0.alert.enabled
            })
        else {
            return
        }
    ...
    private func isInRectangle(_ x: Double, _ y: Double, _ rectangle: BackgroundLandmarkRectangle)
        -> Bool
    {
    ...
    private func executePositionFace(
        ...
        for detection in faceDetections {
            ...
            guard
                let boundingBox = detection.stableBoundingBox(
            ...
            let centerPoint = rotatePoint(
                point: .init(
                    x: centerX - swaeImage.extent.midX, y: centerY - swaeImage.extent.midY),
                alpha: rotationAngle
            )
            outputImage =
                swaeImage
    ...
    override func execute(_ image: CIImage, _ info: VideoEffectInfo) -> CIImage {
        ...
        if let landmarkSettings {
            return executePositionFace(
                image, info.sceneFaceDetections(), alertImage, landmarkSettings)
        } else {
