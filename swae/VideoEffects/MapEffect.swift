...
final class MapEffect: VideoEffect {
    ...
    private func nextNewLocation() -> CLLocation {
        ...
        return newLocations.last(where: { $0.timestamp.advanced(by: delay) <= now }) ?? newLocations
            .first!
    }
    ...
    private func update(size: CGSize) {
        ...
        self.mapSnapshotter?.start(
            with: DispatchQueue.global(),
            completionHandler: { snapshot, error in
    ...
    private func createSnapshotter(newLocation: CLLocation, zoomOutFactor: Int?) -> (
        MKMapSnapshotter, Double
    ) {
    ...
    override func execute(_ image: CIImage, _ info: VideoEffectInfo) -> CIImage {
        ...
        guard let sceneWidget, let dot, let mapSnapshot else {
            // Debug logging to help identify why map widget isn't showing
            if sceneWidget == nil {
                logger.debug("map-widget: No scene widget set")
            }
            if dot == nil {
                logger.debug("map-widget: No dot image available")
            }
            if mapSnapshot == nil {
                logger.debug(
                    "map-widget: No map snapshot available - check location permissions and GPS signal"
                )
            }
            return image
        }
        ...
        return
            ...
            .composited(
                over: applyEffects(mapSnapshot, info)
                    .transformed(
                        by: CGAffineTransform(
