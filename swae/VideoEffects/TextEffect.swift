...
private let textQueue = DispatchQueue(label: "com.suhailget.text")
    ...
    private func formatCheckbox() {
        if checkboxIndex < checkboxes.count {
            parts.append(
                .init(
                    id: partId,
                    data: .imageSystemName(
                        checkboxes[checkboxIndex] ? "checkmark.square" : "square")
                ))
        }
        checkboxIndex += 1
    }
            ...
            for time in lapTimes[lapTimesIndex] {
                ...
                if time.isInfinite {
                    ...
                    let time = ContinuousClock.Duration(
                        secondsComponent: Int64(time), attosecondsComponent: 0)
...
final class TextEffect: VideoEffect {
    ...
    private func formatted(now: ContinuousClock.Instant) -> [Line] {
        guard
            let stats =
                stats
                .last(where: { $0.timestamp.advanced(by: .seconds(delay - 1)) <= now })
                ?? stats
                .first
        else {
            return []
    ...
    private func updateOverlay(size: CGSize) {
        ...
        let (x, y, horizontalAlignment, verticalAlignment, forceUpdate, newImagePresent) =
            textQueue.sync {
                if self.image != nil {
                    newImage = self.image
                    self.image = nil
