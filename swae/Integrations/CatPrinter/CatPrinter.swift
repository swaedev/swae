...
private let catPrinterDispatchQueue = DispatchQueue(label: "com.suhail.cat-printer")
...
class CatPrinter: NSObject {
    ...
    private func printInternal(image: CIImage, feedPaperDelay: Double?) {
        ...
        printJobs.append(
            PrintJob(image: image, feedPaperDelay: feedPaperDelay, printMode: .blackAndWhite))
        tryPrintNext()
    }
    ...
    private func tryPrintNextMxw01(printJob: PrintJob, image: [[UInt8]], peripheral: CBPeripheral) {
        let data = catPrinterPackPrintImageCommandsMxw01(
            image: image, printMode: printJob.printMode)
    ...
    private func playMeowSound() {
        guard let soundUrl = Bundle.main.url(forResource: "Alerts.bundle/Nya", withExtension: "mp3")
        else {
            return
        }
    ...
    private func feedPaper() {
        ...
        send(
            command: .feedPaper(pixels: catPrinterFeedPaperPixels), peripheral, printCharacteristic)
    }
}
...
extension CatPrinter: CBPeripheralDelegate {
    ...
    private func handleMessageMxw01WaitingForReady(
        command: CatPrinterCommandMxw01, currentJob: CurrentJob
    ) {
        guard let peripheral, let printCharacteristic else {
            return
        }
        ...
        case .statusResponse:
            ...
            send(
                command: .printRequest(printMode: currentJob.printMode, count: lineCount),
                peripheral,
                printCharacteristic)
        default:
            break
