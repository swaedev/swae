...
private let cyclingPowerDeviceDispatchQueue = DispatchQueue(
    label: "com.suhail.cycling-power-device")
...
struct CyclingPowerVector {
    ...
    init(value: Data) throws {
        ...
        let value =
            (flags & vectorInstantaneousMeasurementDirectionMask)
            >> vectorInstantaneousMeasurementDirectionIndex
        instantaneousMeasurementDirection = CyclingPowerInstantaneousMeasurementDirection(
            rawValue: value)
    }
}
...
extension CyclingPowerDevice: CBPeripheralDelegate {
    ...
    private func handlePowerMeasurement(value: Data) throws {
        ...
        delegate?.cyclingPowerStatus(
            self, power: averagePower.average(), cadence: averageCadence.averageIngoreZeros())
    }
