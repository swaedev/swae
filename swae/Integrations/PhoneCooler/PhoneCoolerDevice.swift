...
private let phoneCoolerDeviceDispatchQueue = DispatchQueue(label: "com.suhail.phone-cooler-device")
...
extension PhoneCoolerDevice: CBPeripheralDelegate {
    ...
    func setLedColor(color: RgbColor, brightness: Int) {
        ...
        guard
            let setColorCommand = BlackSharkLib.getSetLEDColorCommand(
                color.red,
                color.green,
                color.blue,
    ...
    func turnLedOff() {
        ...
        peripheral.writeValue(
            BlackSharkLib.getTurnOffLEDCommand(), for: writeCharacteristic, type: .withoutResponse)
    }
    ...
    func peripheral(
        ...
        case CBUUID(data: BlackSharkLib.getReadCharacteristicsUUID()):
            ...
            if let coolingState = message as? BlackSharkLib.CoolingState {
                ...
                logger.debug(
                    "phone-cooler-device: Got unknown message \(unknown.rawData.hexString())")
            }
        default:
            break
        }
