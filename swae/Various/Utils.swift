...
extension AVCaptureDevice {
    ...
    func getUIZoomRange(hasUltraWideCamera: Bool) -> (Float, Float) {
        ...
        return (
            Float(minAvailableVideoZoomFactor) * factor, Float(maxAvailableVideoZoomFactor) * factor
        )
    }
...
private let thumbnailQueue = DispatchQueue(label: "com.suhail.swae.thumbnail")
...
func factorToIso(device: AVCaptureDevice, factor: Float) -> Float {
    var iso =
        device.activeFormat.minISO + (device.activeFormat.maxISO - device.activeFormat.minISO)
        * factor
        .clamped(to: 0...1)
    if !iso.isFinite {
        iso = 0
...
func factorToWhiteBalance(device: AVCaptureDevice, factor: Float)
...
func factorFromWhiteBalance(device: AVCaptureDevice, gains: AVCaptureDevice.WhiteBalanceGains)
    -> Float
{
...
extension AVCaptureDevice.WhiteBalanceGains {
    func clamped(maxGain: Float) -> AVCaptureDevice.WhiteBalanceGains {
        return .init(
            redGain: redGain.clamped(to: 1...maxGain),
...
extension NWConnection.ContentContext {
    func webSocketOperation() -> NWProtocolWebSocket.Opcode? {
        let definitions =
            ...
            as? Network.NWProtocolWebSocket
            .Metadata
...
func getAvailableDiskSpace() -> UInt64? {
    guard
        let attributes = try? FileManager.default.attributesOfFileSystem(
            forPath: URL.homeDirectory.path())
    else {
        return nil
    }
