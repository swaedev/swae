...
private let replayQueue = DispatchQueue(label: "com.suhail.replay", qos: .background)
...
protocol ReplayDelegate: AnyObject {
    func replayOutputFrame(
        image: UIImage, offset: Double, video: ReplayBufferFile, completion: (() -> Void)?)
}
...
class ReplayFrameExtractor {
    ...
    init(
        video: ReplayBufferFile, offset: Double, delegate: ReplayDelegate, completion: (() -> Void)?
    ) {
        self.video = video
        self.delegate = delegate
...
extension ReplayFrameExtractor: JobDelegate {
    func jobCompleted(image: UIImage?, video: ReplayBufferFile, offset: Double) {
        if let image {
            delegate?.replayOutputFrame(
                image: image, offset: offset, video: video, completion: completion)
            completion = nil
        }
        job = nil
        tryNextJob()
