...
extension Model {
    ...
    private func handleChatBotMessageHelp() {
        sendChatMessage(
            message: """
                Swae chat bot help: \
                https://github.com/eerimoq/swae/blob/main/docs/chat-bot-help.md#swae-chat-bot-help
                """
        )
    }
    ...
    private func handleChatBotMessageReaction(command: ChatBotCommand) {
        ...
        executeIfUserAllowedToUseChatBot(
            ...
            for device in self.getBuiltinCameraDevices(scene: scene, sceneDevice: self.cameraDevice)
                .devices
