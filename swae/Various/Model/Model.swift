...
final class Model: NSObject, ObservableObject, @unchecked Sendable {
    ...
    var quickButtonChat = ChatProvider(
        maximumNumberOfMessages: maximumNumberOfInteractiveChatMessages)
    ...
    func updateAdaptiveBitrateSrt(stream: SettingsStream) {
        ...
        case .fastIrl:
            ...
            settings.packetsInFlight = Int64(
                stream.srt.adaptiveBitrate!.fastIrlSettings!.packetsInFlight)
            settings
                .minimumBitrate = Int64(
                    stream.srt.adaptiveBitrate!.fastIrlSettings!.minimumBitrate! * 1000)
        ...
        case .belabox:
            ...
            settings
                .minimumBitrate = Int64(
                    stream.srt.adaptiveBitrate!.belaboxSettings!.minimumBitrate * 1000)
            media.setAdaptiveBitrateSettings(settings: settings)
        }
    }
    ...
    func makeErrorToast(
        title: String, font: Font? = nil, subTitle: String? = nil, vibrate: Bool = false
    ) {
        toast.toast = AlertToast(
            type: .regular,
            title: title,
    ...
    func formatLog(log: Deque<LogEntry>) -> URL {
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("Swae-log-\(Date())")
    ...
    func setup() {
        ...
        chatTextToSpeech.setPauseBetweenMessages(
            value: database.chat.textToSpeechPauseBetweenMessages)
        ...
        SwaeShortcuts.updateAppShortcutParameters()
    ...
    @objc func applicationDidChangeActive(notification: NSNotification) {
        ...
        if isAppActive && database.location.enabled {
            locationManager.requestPermissionIfNeeded()
        }
    }
    ...
    func updateFaceFilterButtonState() {
        ...
        if database.debug.beautyFilterSettings.showSwae {
            isOn = true
        }
    ...
    private func handleIpStatusUpdate(statuses: [IPMonitor.Status]) {
        ...
        for status in statuses where status.interfaceType == .wiredEthernet {
            ...
            where !stream.srt.connectionPriorities!.priorities.contains(where: {
                $0.name == status.name
            }) {
        ...
        case .off:
            ...
            if showBackgroudStreamingDisabledToast {
                makeStreamEndedToast(
                    subTitle: String(localized: "Tap this toast to enable background streaming.")
                ) {
    ...
    private func updateAvailableDiskSpace() {
        ...
        if available < 1_000_000_000 {
            stopRecording(
                toastTitle: String(localized: "‼️ Low on disk. Stopping recording. ‼️"),
    ...
    private func removeUnusedAlertMedias() {
        for mediaId in alertMediaStorage.ids() {
            ...
            for widget in database.widgets where widget.type == .alerts {
                for command in widget.alerts.chatBot!.commands
                where command.imagePlaygroundImageId! == mediaId {
                    found = true
                    break
                }
            }
    ...
    func updateAlertsSettings() {
        for widget in database.widgets where widget.type == .alerts {
            widget.alerts.needsSubtitles = !widget.alerts.speechToText!.strings.filter {
                $0.alert.enabled
            }.isEmpty
    ...
    func startWorkout(type: WatchProtocolWorkoutType) {
        guard WCSession.default.isWatchAppInstalled else {
            makeToast(title: String(localized: "Install Swae on your Apple Watch"))
            return
        }
        ...
        makeToast(
            ...
            subTitle: String(localized: "Open Swae in your Apple Watch to start it")
        )
    }
    ...
    func stopWorkout(showToast: Bool = true) {
        ...
        if showToast {
            makeToast(
                title: String(localized: "Ending workout"),
                subTitle: String(localized: "Open Swae in your Apple Watch to end it"))
        }
    }
    ...
    func isAfreecaTvChatConfigured() -> Bool {
        return database.chat.enabled && stream.afreecaTvChannelName != ""
    ...
    func isOpenStreamingPlatformChatConfigured() -> Bool {
        return database.chat.enabled && stream.openStreamingPlatformUrl != ""
            && stream
    ...
    private func logStatus() {
        if logger.debugEnabled, isLive {
            logger.debug(
                "Status: Bitrate: \(bitrate.speedAndTotal), Uptime: \(streamUptime.uptime)")
        }
    }
    ...
    func padelScoreboardSettingsToEffect(_ scoreboard: SettingsWidgetPadelScoreboard)
        -> PadelScoreboard
    {
    ...
    func findScoreboardPlayer(id: UUID) -> String {
        return database.scoreboardPlayers.first(where: { $0.id == id })?.name ?? "🇸🇪 Swae"
    }
        ...
        if battery.level <= 0.07, !isBatteryCharging(), !isMac() {
            ...
            if (battery.levelLowCounter % 3) == 0 {
                ...
                if database.chat.botEnabled, database.chat.botSendLowBatteryWarning {
                    sendChatMessage(message: "Swae bot: \(lowBatteryMessage)")
                }
            }
        } else {
            battery.levelLowCounter = -1
    ...
    private func updateRtmpIngestsSpeed(
        _ anyServerEnabled: inout Bool,
    ...
    private func updateSrtlaIngestsSpeed(
        _ anyServerEnabled: inout Bool,
        _ speed: inout UInt64,
        _ total: inout UInt64,
        _ numberOfClients: inout Int
    ...
    private func updateRistIngestsSpeed(
        _ anyServerEnabled: inout Bool,
        _ speed: inout UInt64,
        _ total: inout UInt64,
        _ numberOfClients: inout Int
    ...
    private func updateRtspIngestsSpeed(
        _ anyServerEnabled: inout Bool,
        _ speed: inout UInt64,
        _ total: inout UInt64,
        _ numberOfClients: inout Int
    ...
    func attachBackTripleLowEnergyCamera(force: Bool = true) {
        ...
        zoom.x = zoom.backX
        guard
            let bestDevice = AVCaptureDevice.default(
        ...
        cameraDevice = device
        cameraZoomLevelToXScale = device.getZoomFactorScale(
            hasUltraWideCamera: hasUltraWideBackCamera())
        (cameraZoomXMinimum, cameraZoomXMaximum) =
            bestDevice
            .getUIZoomRange(hasUltraWideCamera: hasUltraWideBackCamera())
    ...
    func attachCamera(scene: SettingsScene, position: AVCaptureDevice.Position) {
        ...
        cameraZoomLevelToXScale =
            cameraDevice?.getZoomFactorScale(hasUltraWideCamera: hasUltraWideBackCamera()) ?? 1.0
        (cameraZoomXMinimum, cameraZoomXMaximum) =
            cameraDevice?
    ...
    private func getIgnoreFramesAfterAttachSeconds() -> Double {
        return Double(database.debug.cameraSwitchRemoveBlackish)
            + database.debug.builtinAudioAndVideoDelay
    }
    ...
    func isStatusBrowserWidgetsActive() -> Bool {
        return !statusTopRight.browserWidgetsStatus.isEmpty
            && statusTopRight.browserWidgetsStatusChanged
    }
}
