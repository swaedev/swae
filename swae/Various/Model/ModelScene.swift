...
extension Model {
    ...
    private func unregisterGlobalVideoEffects() {
        ...
        faceEffect = FaceEffect(
            fps: Float(stream.fps), onFindFaceChanged: handleFindFaceChanged(value:))
    ...
    func getVideoSourceEffect(id: UUID) -> VideoSourceEffect? {
        for (videoSourceEffectId, videoSourceEffect) in videoSourceEffects
        where id == videoSourceEffectId {
    ...
    private func resetVideoEffects(widgets: [SettingsWidget]) {
        ...
        for widget in widgets where widget.type == .browser {
            ...
            let browserEffect = BrowserEffect(
                ...
                videoSize: videoSize,
                settingName: widget.name,
                swaeAccess: widget.browser.swaeAccess
            )
    ...
    private func isFaceEnabled() -> Bool {
        ...
        return database.debug.beautyFilter || settings.showBlur || settings.showBlurBackground
            || settings
                .showSwae
            || settings.showBeauty
    }
    ...
    func sceneUpdated(
        imageEffectChanged: Bool = false, attachCamera: Bool = false, updateRemoteScene: Bool = true
    ) {
    ...
    private func sceneUpdatedOn(scene: SettingsScene, attachCamera: Bool) {
        ...
        for snapshotEffect in snapshotEffects.values
        where !enabledSnapshotEffects.contains(snapshotEffect) {
    ...
    private func addSceneEffects(
        ...
        for sceneWidget in scene.widgets {
            ...
            case .crop:
                ...
                {
                    browserEffect.setSceneWidget(
                        sceneWidget: findSceneWidget(
                            scene: scene, widgetId: widget.crop.sourceWidgetId),
                        crops: findWidgetCrops(
                            scene: scene, sourceWidgetId: widget.crop.sourceWidgetId)
                    )
                    if !browserEffect.audioOnly {
                        effects.append(browserEffect)
                    }
            ...
            case .scene:
                if let sceneWidgetScene = getLocalAndRemoteScenes().first(where: {
                    $0.id == widget.scene.sceneId
                }) {
                    addSceneEffects(
                        sceneWidgetScene,
                        &effects,
            ...
            case .videoSource:
                if let videoSourceEffect = videoSourceEffects[widget.id] {
                    if let videoSourceId = getVideoSourceId(
                        cameraId: widget.videoSource.toCameraId())
                    {
    ...
    private func findWidgetCrops(scene: SettingsScene, sourceWidgetId: UUID) -> [WidgetCrop] {
        ...
        for sceneWidget in scene.widgets.filter({ $0.enabled }) {
            ...
            crops.append(
                WidgetCrop(
                    position: .init(x: sceneWidget.x, y: sceneWidget.y),
                    crop: .init(
                        x: crop.x,
                        y: crop.y,
                        width: crop.width,
    ...
    private func updateTextWidgetsLapTimes(now: Date) {
        for widget in database.widgets where widget.type == .text {
            ...
            getTextEffect(id: widget.id)?.setLapTimes(
                lapTimes: widget.text.lapTimes.map { $0.lapTimes })
        }
    }
    ...
    func getBuiltinCameraDevices(scene: SettingsScene, sceneDevice: AVCaptureDevice?)
        -> CaptureDevices
    {
    ...
    private func getBuiltinCameraDevicesInScene(
        scene: SettingsScene, devices: inout [CaptureDevice]
    ) {
        for sceneWidget in scene.widgets {
            guard let widget = findWidget(id: sceneWidget.widgetId) else {
                continue
            ...
            switch widget.type {
            case .videoSource:
                getBuiltinCameraDevicesForVideoSourceWidget(
                    videoSource: widget.videoSource, devices: &devices)
            case .vTuber:
                getBuiltinCameraDevicesForVTuberWidget(vTuber: widget.vTuber, devices: &devices)
            case .pngTuber:
                getBuiltinCameraDevicesForPngTuberWidget(
                    pngTuber: widget.pngTuber, devices: &devices)
            case .scene:
    ...
    private func getBuiltinCameraDevicesForVTuberWidget(
        vTuber: SettingsWidgetVTuber, devices: inout [CaptureDevice]
    ) {
        let cameraId: String?
    ...
    private func getBuiltinCameraDevicesForSceneWidget(
        scene: SettingsWidgetScene, devices: inout [CaptureDevice]
    ) {
    ...
    func switchToNextSceneRoundRobin() {
        guard let currentSceneIndex = findEnabledSceneIndex(id: sceneSelector.selectedSceneId)
        else {
            return
        }
