...
private let rtspClientQueue = DispatchQueue(label: "com.suhail.swae.rtsp")
...
private class SdpLinesParser {
    ...
    private func parseMedia(value: String, linesReader: SdpLinesReader) throws {
        ...
        var mediaDescription = SdpMediaDescription(
            media: String(parts[0]),
            port: String(parts[1]),
...
private class RtpProcessorVideoH265: RtpVideoProcessor {
    ...
    private func processBufferTypeFu(packet: Data, timestamp: UInt64) throws {
        ...
        if startBit == 1 {
            tryDecodeFrame()
            startNewFrame(
                timestamp: timestamp, first: Data([nal, packet[13]]), second: packet[15...])
        } else {
            data += packet[15...]
        }
    }
...
class RtspClient {
    ...
    private func handleUnauthorizedResponse(request: Request, response: Response) throws {
        guard !request.dueToAuthenticationFailure else {
            delegate?.rtspClientErrorToast(
                title: String(localized: "Wrong RTSP username or password"))
            throw "Wrong username or password"
        }
        guard username != nil, password != nil else {
            delegate?.rtspClientErrorToast(
                title: String(localized: "RTSP username or password missing"))
    ...
    private func performOptions() {
        perform(
            request: Request(method: "OPTIONS", url: url) { _ in
    ...
    private func performGetParameter() {
        perform(
            request: Request(method: "GET_PARAMETER", url: url) { _ in
                self.isAlive = true
            })
    }
    ...
    private func performDescribe() {
        ...
        perform(
            request: Request(method: "DESCRIBE", url: url, headers: headers) { response in
                try self.handleDescribeResponse(response: response)
            })
    }

    private func handleDescribeResponse(response: Response) throws {
        guard let content = response.content, let content = String(data: content, encoding: .utf8)
        else {
    ...
    private func setupH264(sdpVideo: SdpVideoH264) throws {
        ...
        rtpVideo.processor = RtpProcessorVideoH264(
            formatDescription: formatDescription, client: self)
    }
    ...
    private func setupH265(sdpVideo: SdpVideoH265) throws {
        ...
        rtpVideo.processor = RtpProcessorVideoH265(
            formatDescription: formatDescription, client: self)
    }
    ...
    private func performSetup(url: URL) {
        ...
        perform(
            request: Request(method: "SETUP", url: url, headers: headers) { response in
    ...
    private func performPlay() throws {
        ...
        perform(
            request: Request(method: "PLAY", url: url, headers: headers) { response in
