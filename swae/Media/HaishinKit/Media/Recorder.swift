...
private let recorderQueue = DispatchQueue(label: "com.suhail.recorder")
...
class Recorder: NSObject {
    ...
    func startRunning(
        url: URL?, replay: Bool, audioOutputSettings: [String: Any],
        ...
        guard let writer,
            ...
            let sampleBuffer =
                ...
                .replacePresentationTimeStamp(
                    sampleBuffer.presentationTimeStamp - basePresentationTimeStamp)
        else {
            return
        }
    ...
    private func tryConvertAudio(_ sampleBuffer: CMSampleBuffer, makeConverter: Bool = false)
        -> CMSampleBuffer?
    {
        if makeConverter {
        ...
        if let sourceFormatHint, let inSourceFormat = sourceFormatHint.audioStreamBasicDescription {
            for (key, value) in audioOutputSettings {
                ...
                case AVNumberOfChannelsKey:
                    outputSettings[key] =
                        isZero(value) ? min(Int(inSourceFormat.mChannelsPerFrame), 2) : value
                default:
    ...
    private func makeWriterInput(
        ...
        if let audioStreamBasicDescription = sampleBuffer.formatDescription?
    ...
    private func makeChannelLayout(_ numberOfChannels: UInt32) -> AVAudioChannelLayout? {
        ...
        return AVAudioChannelLayout(
            layoutTag: kAudioChannelLayoutTag_DiscreteInOrder | numberOfChannels)
    }

    private func makeAudioFormat(_ basicDescription: inout AudioStreamBasicDescription)
        -> AVAudioFormat?
    {
    ...
    private func startRunningInner(
        ...
        guard writer == nil else {
            logger.info(
                "recorder: Will not start recording as it is already running or missing URL")
            return
        }
        reset()
    ...
    private func stopRunningInner() {
        ...
        guard writer.status == .writing else {
            logger.info(
                "recorder: Failed to finish writing \(writer.error?.localizedDescription ?? "")")
            reset()
            return
        }
            ...
            if self.replay {
                ...
                case .separable:
                    if let report = segmentReport?.trackReports.first {
                        self.delegate?.recorderDataSegment(
