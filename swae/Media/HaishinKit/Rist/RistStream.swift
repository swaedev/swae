...
private let ristQueue = DispatchQueue(label: "com.suhailwae.rist")
...
private class RistRemotePeer: AdaptiveBitrateDelegate {
    ...
    init(
        ...
        adaptiveWeight = AdaptiveBitrateRistExperiment(
            targetBitrate: weigthTargetBitrate, delegate: self)
...
class RistStream {
    ...
    func connectionStatistics() -> [BondingConnection] {
        ...
        ristQueue.sync {
            for peer in peers {
                var connection = BondingConnection(
                    name: peer.bondingConnectionName(), usage: 0, rtt: nil)
                if let stats = peer.stats {
    ...
    private func addRelayInner(endpoint: NWEndpoint, id _: UUID, name: String) {
        ...
        addPeer(
            url: makeBondingUrl("rist://\(endpoint)"),
            interfaceName: name,
            interfaceType: nil,
    ...
    private func handleNetworkPathUpdate(path: NWPath) {
        ...
        for interface in interfaces {
            ...
            addPeer(
                url: makeBondingUrl(url, interface.name),
