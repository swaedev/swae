...
let srtlaServerQueue = DispatchQueue(label: "com.suhail.srtla-server", qos: .userInitiated)
...
class SrtlaServer {
    ...
    init(settings: SettingsSrtlaServer, timecodesEnabled: Bool) {
        ...
        srtServer = SrtServer(
            timecodesEnabled: timecodesEnabled, port: settings.srtlaSrtPort(), srtlaPatches: true)
    ...
    private func handleSrtlaControlPacket(
        connection: NWConnection, type: SrtlaPacketType,
        packet: Data
    ) -> Bool {
        switch type {
        case .reg1:
    ...
    private func handleSrtlaReg1(connection: NWConnection, packet: Data) {
        ...
        let groupId =
            packet[srtControlTypeSize..<srtControlTypeSize + 128] + Data.random(length: 128)
