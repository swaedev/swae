...
class SrtServer {
    ...
    func start() {
        srt_startup()
        running = true
        DispatchQueue(label: "com.suhail.srtla-srt-server", qos: .userInteractive).async {
            do {
                try self.main()
            } catch {
