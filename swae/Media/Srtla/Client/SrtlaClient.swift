...
let srtlaClientQueue = DispatchQueue(label: "com.suhail.srtla-client")
...
class SrtlaClient: NSObject {
    ...
    func removeMoblink(endpoint: NWEndpoint) {
        ...
        srtlaClientQueue.async {
            ...
            guard
                let remoteConnection = self.remoteConnections
            ...
            for connection in self.remoteConnections {
                if let relayId = connection.relayId {
                    connection.setPriority(
                        priority: self.getRelayConnectionPriority(relayId: relayId))
                } else {
    ...
    private func updateConnectionPriorities(
        connectionPriorities: SettingsStreamSrtConnectionPriorities
    ) {
        self.connectionPriorities = .init()
        ...
        guard
            let lowestPriority = connectionPriorities.priorities
    ...
    private func getConnectionPriority(name: String) -> Float {
        guard
            let priority = connectionPriorities.first(where: { connection in
    ...
    private func getRelayConnectionPriority(relayId: UUID) -> Float {
        guard
            let priority = connectionPriorities.first(where: { priority in
    ...
    private func handleNetworkPathUpdate(path: NWPath) {
        ...
        for interface in path.availableInterfaces where interfaceTypes.contains(interface.type) {
            guard
                !newRemoteConnections.contains(where: { connection in
    ...
    private func startRemote(
        connection: RemoteConnection, host: NWEndpoint.Host, port: NWEndpoint.Port
    ) {
