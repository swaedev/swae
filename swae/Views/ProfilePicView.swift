...
struct ProfilePicView: View {
    ...
    init(
        pubkey: String, size: CGFloat, profile: UserMetadata?, picture: String? = nil,
        show_zappability: Bool? = nil
    ) {
        self.pubkey = pubkey
    ...
    var body: some View {
        ZStack(alignment: Alignment(horizontal: .trailing, vertical: .bottom)) {
            InnerProfilePicView(
                url: profile?.pictureURL, fallbackUrl: URL(string: robohash(pubkey)),
                pubkey: pubkey, size: size)
