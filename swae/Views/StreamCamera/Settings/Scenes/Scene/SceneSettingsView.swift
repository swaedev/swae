                    ...
                    VStack {
                        ...
                        let form = Form {
                            if widgets.isEmpty {
                                Section {
                                    Text(
                                        "No widgets found. Create widgets in Settings → Scenes → Widgets."
                                    )
                                }
                            } else {
                                Section("Widget name") {
                                    ForEach(widgets) { widget in
                                        Button {
                                            ...
                                            // Request location permission if adding a map widget
                                            if widget.type == .map {
                                                model.locationManager.requestPermissionIfNeeded()
                                                if !model.database.location.enabled {
                                                    // Also enable location in app settings if not already enabled
                                            ...
                                            var attachCamera = false
                                            if scene.id == model.getSelectedScene()?.id {
                                                attachCamera = model.isCaptureDeviceWidget(
                                                    widget: widget)
                                            }
                                            model.sceneUpdated(
                                                imageEffectChanged: true, attachCamera: attachCamera
                                            )
                                            showingAddWidget = false
                                        } label: {
                                            IconAndTextView(
