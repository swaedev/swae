                ...
                .gesture(
                    DragGesture()
                        .onChanged { value in
                            ...
                            let size = CGSize(
                                width: reader.size.width, height: reader.size.height - 12)
    ...
    var body: some View {
        Form {
            ...
            .onChange(of: comparisonOperator) { _ in
                let comparisonOperator = SettingsWidgetAlertsCheerBitsAlertOperator(
...
private struct TwitchRewardView: View {
    ...
    var body: some View {
        Form {
            ...
            AlertMediaView(
                alert: reward.alert, imageId: reward.alert.imageId, soundId: reward.alert.soundId)
        }
    ...
    var body: some View {
        NavigationLink {
            Form {
                ...
                Section {
                    TextEditNavigationView(
                        title: String(localized: "Name"), value: name, onSubmit: onSubmit)
                } footer: {
                    Text("Trigger with chat message '!swae alert \(name)'")
                }
                // Section {
    ...
    var body: some View {
        NavigationLink {
            Form {
                ...
                Section {
                    TextEditNavigationView(
                        title: String(localized: "String"), value: text, onSubmit: onSubmit)
                } footer: {
                ...
                List {
                    ForEach(speechToText.strings) { string in
                        SpeechToTextStringView(
                            alert: string.alert, string: string, text: string.string)
                    }
                    .onDelete { indexes in
