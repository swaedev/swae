...
private struct WidgetScoreboardPlayerView: View {
    @EnvironmentObject var model: Model
...
struct WidgetScoreboardSettingsView: View {
    ...
    var body: some View {
        ...
        Section {
            WidgetScoreboardPlayerView(playerId: $homePlayer1)
                .onChange(of: homePlayer1) { _ in
                    widget.scoreboard.padel.homePlayer1 = homePlayer1
                    model.resetSelectedScene(changeScene: false)
                }
            if gameType == .doubles {
                WidgetScoreboardPlayerView(playerId: $homePlayer2)
        ...
        Section {
            WidgetScoreboardPlayerView(playerId: $awayPlayer1)
                .onChange(of: awayPlayer1) { _ in
                    widget.scoreboard.padel.awayPlayer1 = awayPlayer1
                    model.resetSelectedScene(changeScene: false)
                }
            if gameType == .doubles {
                WidgetScoreboardPlayerView(playerId: $awayPlayer2)
