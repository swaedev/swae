        ...
        Form {
            Section {
                Toggle("Enabled", isOn: $location.enabled)
                    .onChange(of: location.enabled) { enabled in
                        if enabled {
                            // Proactively request permission when enabling location
                ...
                Text(
                    "Both settings must be enabled for location features to work. If iOS permission is denied, enable it in iOS Settings → Privacy & Security → Location Services."
                )
            }
            if database.showAllSettings {
                Section {
                    ...
                    Picker("Distance filter", selection: $location.distanceFilter) {
                        ForEach(SettingsLocationDistanceFilter.allCases, id: \.self) {
                            distanceFilter in
