...
struct PhoneCoolerDeviceSettingsView: View {
    ...
    private func onDeviceChange(value: String) {
        ...
        guard
            let peripheral = scanner.discoveredPeripherals.first(where: {
    ...
    private func changeColor() {
        let phoneCoolerDevice = model.phoneCoolerDevices.first(where: {
            $0.key == device.bluetoothPeripheralId
        })?.value
    ...
    private func toggleLight() {
        guard
            let phoneCoolerDevice = model.phoneCoolerDevices
        ...
        if device.rgbLightEnabled {
            phoneCoolerDevice.setLedColor(
                color: device.rgbLightColor, brightness: Int(device.rgbLightBrightness))
        } else {
        ...
        NavigationLink {
            Form {
                ...
                Section {
                    NavigationLink {
                        PhoneCoolerDeviceScannerSettingsView(
                            ...
                            selectedId: device.bluetoothPeripheralId?.uuidString
                                ?? String(localized: "Select device")
                        )
                    } label: {
                    ...
                    if let phoneTemp = status.phoneCoolerPhoneTemp,
                        let exhaustTemp = status.phoneCoolerExhaustTemp
                    {
                        HStack {
                ...
                Section {
                    ...
                    if device.rgbLightEnabled {
                        ColorPicker(
                            "Color", selection: $device.rgbLightColorColor, supportsOpacity: false
                        )
