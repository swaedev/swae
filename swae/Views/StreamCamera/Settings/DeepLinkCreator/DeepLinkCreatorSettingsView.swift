...
private let defaultDeepLink = "swae://?{}"
...
struct DeepLinkCreatorSettingsView: View {
    ...
    private func createDeepLinkStream(stream: DeepLinkCreatorStream) -> SwaeSettingsUrlStream {
        let newStream = SwaeSettingsUrlStream(name: stream.name, url: stream.url)
        if stream.selected {
    ...
    private func updateDeepLinkStreams(settings: SwaeSettingsUrl) {
    ...
    private func updateDeepLinkQuickButtons(settings: SwaeSettingsUrl) {
        ...
        for button in deepLinkCreator.quickButtons.buttons where button.enabled {
            ...
            let newButton = SwaeSettingsButton(type: button.type)
            newButton.enabled = true
            settings.quickButtons!.buttons!.append(newButton)
        }
    }

    private func updateDeepLinkWebBrowser(settings: SwaeSettingsUrl) {
                ...
                Section {
                    ...
                    NavigationLink {
                        DeepLinkCreatorQuickButtonsSettingsView(
                            quickButtons: deepLinkCreator.quickButtons)
                    } label: {
                    ...
                    NavigationLink {
                        DeepLinkCreatorWebBrowserSettingsView(
                            webBrowser: deepLinkCreator.webBrowser)
                    } label: {
                    ...
                    Section {
                        HCenter {
                            Button("Copy to clipboard") {
                                ...
                                model.makeToast(
                                    title: String(localized: "Deep link copied to clipboard"))
                            }
                        }
                    }
                    Section {
